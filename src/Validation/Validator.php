...
namespace EduSoho\Framework\Validation;
...
class Validator 
{
	 ...
	 * @param  array  $rules  
	 * @param  array  $config 
	 * @return void get errors from validator.errors, get filtered Data from validator.filteredData
	 *  
	 */
	...
	private function validateByRule(array $data, $key, $val, $rule){
		...
		}else if($this->defaultConfig["ignoreSyntaxError"]){
	 ...
	 * @return boolean     true = is variable
	 */
	...
	private function isNumber($value)
	{
		...
		if(is_numeric($value) && is_string($value)) return $this->match("/^\d+(\.?)\d+$/", $value);
	}
